...
namespace QLTT.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var crawler = new SuKienCrawler();
            
            var urls = new List<string>
    {
        "https://vnexpress.net/giai-bong-da-cac-giao-si-viet-nam-4494168.html",
        "https://vnexpress.net/khoi-tranh-giai-bong-da-thieu-nhi-lotteria-challenge-cup-2023-4652189.html",
        "https://vnexpress.net/vngroup-to-chuc-giai-bong-da-giao-huu-doanh-nghiep-4644778.html",
        "https://vnexpress.net/giai-bong-da-nhi-dong-u11-toan-quoc-khoi-tranh-tu-25-5-4603703.html"
    };
        ...
        public ActionResult Result(String maKH, String ngayThue, int soGioThue, String maSan)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (maSan.StartsWith("["))
                    {
                        fieldIds = JsonConvert.DeserializeObject<List<int>>(maSan);
                    }
                    else
                    {
                        fieldIds = new List<int> { int.Parse(maSan) };
                    }

                    DateTime parsedRentDate = DateTime.ParseExact(ngayThue, "d/M/yyyy", CultureInfo.InvariantCulture).AddHours(12);

                    var rentCommand = new RentFieldCommand(db, fieldIds, maKH, parsedRentDate, DateTime.Now, soGioThue);

                    CommandInvoker invoker = new CommandInvoker();

                    invoker.SetCommand(rentCommand);
