...
namespace QLTT.Controllers.Home
{
    ...
    {
        public static void SendEmail(string recipientEmail, string subject, string htmlMessage)
        {
            var smtpClient = new SmtpClient("smtp.gmail.com", 587);
            smtpClient.Credentials = new System.Net.NetworkCredential("testqlttsup@gmail.com", "cqbg kgem erba xxfn");
            smtpClient.EnableSsl = true;
            var mailMessage = new MailMessage("testqlttsup@gmail.com", recipientEmail);
        ...
        public ActionResult DeleteVoteRent(int? id)
        {
            ...
            if (tblPhieuDatsan == null)
            {
                return HttpNotFound();
            }
            if (tblPhieuDatsan.maKH != kh.maKH)
                return RedirectToAction("Index", "Home");
        ...
        public ActionResult ConfirmDeleteVoteRent(int id)
        {
            try
            {
                ...
                var cancelCommand = new CancelRentFieldCommand(db, id);
                var commandInvoker = new CommandInvoker();

                commandInvoker.SetCommand(cancelCommand);
            ...
            catch (Exception ex)
            {
                ...
                ModelState.AddModelError("", "\r\nĐã xảy ra lỗi khi hủy . Vui lòng thử lại.");
               
                return View("DeleteVoteRent", id);
            }
        }
        ...
        private void AutoCancelVoteRent()
        {
            ...
            foreach (var item in tblPhieuDatSan)
            {
        ...
        public ActionResult DetailsBill(int? id)
        {
            ...
            int so_gio_thue = tblHoaDon.PhieuThueSan.soGioThue ?? 0;  
            int gia = (int)tblHoaDon.PhieuThueSan.San.LoaiSan.giaThue;
            var tien_San = so_gio_thue  * gia;  
        ...
        {
            var user = db.KhachHangs.FirstOrDefault(u => u.emailKH == email);
            if (user != null)
            {
                ...
                Task.Run(() => SendEmail(user.emailKH, "Mật khẩu mới của bạn", $"Mật khẩu mới là: {newPassword}"));
                return View("ForgotPassConfirmation");
            }
            else
            {
                return View("ForgotPassError"); 
            }
        }
