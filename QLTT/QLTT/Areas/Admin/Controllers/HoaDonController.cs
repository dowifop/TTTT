...
namespace QLTT.Areas.Admin.Controllers.Admin
{
    public class HoaDonController : Controller
    {
        ...
        public ActionResult Extend(int? id)
        {
            ...
            PhieuThueSan pds = db.PhieuThueSans.Find(tblHoaDon.maPT);
            String dt = null;
            try
            {
                int? d = db.PhieuThueSans.Where(t => t.maTinhTrang == 1 && t.maSan == pds.San.maSan).Select(t => t.soGioThue).OrderBy(t => t).First();
                dt = d.ToString();
            }
            catch (Exception e)
            {
        ...
        public ActionResult ResultExtend(String ma_pts, int soGioThue)
        {
            ...
            try
            {
                PhieuThueSan pds = db.PhieuThueSans.Find(Int32.Parse(ma_pts));
                pds.soGioThue = soGioThue;
                ViewBag.result = "success";
                ViewBag.soGioThue = soGioThue;
                db.SaveChanges();
            }
