...
namespace QLTT.Areas.Admin.Controllers.Admin
{
    public class PhieuDatSanController : Controller
    {
        ...
        public ActionResult SelectInfrastructure(String ngayThue, string soGioThue)
        {
            if (ngayThue == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            int hours;
            bool isValidSoGioThue = int.TryParse(soGioThue, out hours);
            ViewBag.soGioThue = isValidSoGioThue ? hours.ToString() : "Lỗi";
            ViewBag.ma_kh = new SelectList(db.KhachHangs, "maKH", "maKH");
            DateTime ngay_ra = (DateTime.Parse(ngayThue)).AddHours(12);
        ...
        public ActionResult Edit([Bind(Include = "maPT,maKH,soGioThue,NgayThue,NgayDat,maSan,maTinhTrang")] PhieuThueSan phieuThueSan)
        {
            ...
            {
                var editCommand = new EditRentFeildCommand(db, phieuThueSan);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            try
            {
                var deleteCommand = new DeleteVoteRentCommand(db, id);
                var commandInvoker = new CommandInvoker();
